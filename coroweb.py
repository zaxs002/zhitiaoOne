...
def has_auth(fn):
    return getattr(fn, '__auth__', True)
    ...
    async def __call__(self, request):
        ...
        if kw is None:
            ...
            for k, v in request.match_info.items():
                if k in kw:
                    logging.warning('named参数和kw参数中有重复的参数名: %s' % k)
                kw[k] = v
        ...
        if self._required_kw_args:
            for name in self._required_kw_args:
                if not name in kw:
                    return web.HTTPBadRequest('没有参数: %s' % name)
        logging.info('带着参数 %s 调用RequestHandler' % str(kw))
        try:
